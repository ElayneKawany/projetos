...
import { Plus, Search, Filter, FolderKanban, TrendingUp, AlertTriangle, Clock } from 'lucide-react'
...
export default function ProjetosClient({ projetos: initial, diretorias, areas, usuarios, session }: Props) {
  ...
  const atrasados = projetos.filter(p =>
    ...
    !['ENCERRAMENTO','CANCELADO','SUSPENSO'].includes(p.status)
  ).length
    ...
    <div className="animate-fade-in">
      ...
      <div className="card mb-4">
        <div className="flex flex-wrap gap-3">
          ...
          {(busca || filtroStatus || filtroPrioridade || filtroDiretoria) && (
            <button className="btn-ghost text-sm" onClick={() => {
              setBusca(''); setFiltroStatus(''); setFiltroPrioridade(''); setFiltroDiretoria('')
            }}>
              Limpar filtros
            </button>
          )}
              ...
              {filtrados.length === 0 ? (
                ...
                >
                  ...
                  <td className="text-sm">
                    {p.data_fim_prev ? (
                      <span className={new Date(p.data_fim_prev) < hoje && !['ENCERRAMENTO','CANCELADO'].includes(p.status) ? 'text-red-500 font-medium' : ''}>
                        {new Date(p.data_fim_prev).toLocaleDateString('pt-BR')}
                      </span>
                    ) : '—'}
                  </td>
