...
Para as próximas fatias (repositórios restantes): mesmo padrão — trocar `db`→`asyncDb`, ajustar nomes de tabela pro schema Postgres real, checar se a tabela é tocada por outros arquivos fora do repositório **e por outros repositórios** (não presumir que só um dono existe), propagar `await`, conferir manualmente arrays/`if` sem `await` (tsc não pega), testar contra o Postgres de dev antes de qualquer commit. Ordem sugerida mantida: repositórios menores primeiro, `cronograma.ts` (mais crítico, tem a lógica de Nova Versão corrigida nesta sessão) por último, com testes manuais extras.
