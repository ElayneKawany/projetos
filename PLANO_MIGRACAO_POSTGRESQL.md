...
Para as próximas fatias (repositórios restantes): mesmo padrão — trocar `db`→`asyncDb`, ajustar nomes de tabela pro schema Postgres real, checar se a tabela é tocada por outros arquivos fora do repositório (não presumir que não é), propagar `await`, testar contra o Postgres de teste antes de qualquer commit. Ordem sugerida mantida: repositórios menores primeiro, `cronograma.ts` (mais crítico, tem a lógica de Nova Versão corrigida nesta sessão) por último, com testes manuais extras.
